...
function optimiseDataValidations(model) {
  ...
  const dvList = _.map(model, (dataValidation, address) => ({
    ...
    .filter(({dataValidation}) => DataValidationsExtXform.isExt(dataValidation))
...
class DataValidationsExtXform extends BaseXform {
  get tag() {
    return 'x14:dataValidations';
  }

  static isExt(dataValidation) {
    if ('x14Id' in dataValidation) {
      return true;
    }
  ...
  hasContent(model) {
    const hasExtContent = Symbol('hasExtContent');
    if (model[hasExtContent] === undefined) {
      model[hasExtContent] = Object.values(model).some(DataValidationsExtXform.isExt);
    }
  ...
  render(xmlStream, model) {
    ...
    if (optimizedModel.length) {
      xmlStream.openNode('x14:dataValidations', {
      ...
      optimizedModel.forEach(value => {
        ...
        // xmlStream.addAttribute('xr:uid', value.x14Id || `{${uuidv4()}}`.toUpperCase());
        ...
        (value.formulae || []).forEach((formula, index) => {
          xmlStream.openNode(`x14:formula${index + 1}`);
        ...
        xmlStream.writeText(value.sqref);
        xmlStream.closeNode();
        xmlStream.closeNode();
      });
      xmlStream.closeNode();
